...
data "aws_ssm_parameter" "eks_al2" {
  for_each = toset(["1.27"])

  name = "/aws/service/eks/optimized-ami/${each.value}/amazon-linux-2/recommended/image_id"
}

data "aws_ssm_parameter" "eks_al2_gpu" {
  for_each = toset(["1.27"])

  name = "/aws/service/eks/optimized-ami/${each.value}/amazon-linux-2-gpu/recommended/image_id"
}
...
resource "local_sensitive_file" "pub_key_pair" {
  content         = module.key_pair.public_key_pem
  filename        = "${path.module}/${module.key_pair.key_pair_name}.pub"
  file_permission = "0400"
}
...
module "ec2_eks_al2" {
  ...
  name = "${local.name}-eks-al2"

  ami                    = data.aws_ssm_parameter.eks_al2["1.27"].value
  instance_type          = "t3.large"
  availability_zone      = element(module.vpc.azs, 0)
  subnet_id              = element(module.vpc.public_subnets, 0)
  vpc_security_group_ids = [module.security_group.security_group_id]
