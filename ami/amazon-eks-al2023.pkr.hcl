locals {
  ...
  ami_name = "${var.ami_name_prefix}-${var.eks_version}-${var.cpu_architecture}-${local.timestamp}"
...
data "amazon-parameterstore" "this" {
  name   = "/aws/service/ami-amazon-linux-latest/al2023-ami-minimal-kernel-6.1-${var.cpu_architecture}"
  region = var.region
}
